...
def download_database():
    sub = db.query(
        ...
        func.row_number().over(partition_by=table.Dish.submenu_id).cast(String).label('submenu_description'),
        table.Dish.title.label('dish_title'),
        table.Dish.description.label('dish_description'),
        table.Dish.price.label('dish_price'),
        ).join(
            table.SubMenu, table.SubMenu.menu_id == table.Menu.id
        ).join(
            table.Dish, table.Dish.submenu_id == table.SubMenu.id
        ).union(
            db.query(
                table.Menu.id,
    ...
    res = db.query(
        ...
        sub.c.dish_price,
            ...
            sub.c.menu_id, sub.c.submenu_id, sub.c.dish_id
        )
