...
async def create_dish(data: Data, menu_id: int, submenu_id: int):
    try:
        ...
        raise _fastapi.HTTPException(status_code=404, detail=f"submenu {submenu_id} not found")
    try:
        menu = db.query(table.Menu).filter_by(id=menu_id).one()
    except Exception:
        raise _fastapi.HTTPException(status_code=404, detail=f"menu {menu_id} not found")
    ...
    await cache_func.cache_update(f"Menu_{menu_id}", {"dishes_count": menu.dishes_count})
    await cache_func.cache_update(f"Menu_{menu_id}_SubMenu_{submenu_id}", {"dishes_count": submenu.dishes_count})
...
async def get_dish(menu_id: int, submenu_id: int, id: int):
    ...
    if dish:
        return dish
    try:
        dish = db.query(table.Dish).join(table.SubMenu).join(table.Menu).filter(table.Menu.id == menu_id, table.Dish.id == id, table.Dish.submenu_id == submenu_id).one()
    except Exception:
        raise _fastapi.HTTPException(
            status_code=404,
