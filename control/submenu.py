...
async def create_submenu(data: Data, menu_id: int):
    ...
    await cache_func.cache_update(f"Menu_{menu_id}", {"submenus_count": menu.submenus_count})
...
async def delete_submenu(menu_id: int, id: int):
    ...
    await cache_func.cache_update(f"Menu_{menu_id}", {"submenus_count": menu.submenus_count, "dishes_count": menu.dishes_count})
    response = Delete
...
async def get_submenu(menu_id: int, id: int):
    ...
    if submenu:
        return submenu
    try:
        submenu = db.query(table.SubMenu).join(table.Menu).filter(table.SubMenu.id == id, table.Menu.id == menu_id).one()
    except Exception:
        raise _fastapi.HTTPException(
            status_code=404,
