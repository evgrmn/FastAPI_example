...
async def get_order_list(db: AsyncSession, user: User):
    ...
    order_list = await db.execute(
        _sql.select(table.Order).filter_by(user_id=user["id"])
    )
    order_list = list(
        map(lambda x: Order.from_orm(x).dict(), order_list.scalars().all())
    )
...
async def manage_order(dish_id: int, quantity: int, db: AsyncSession, user: User):
    all_dish_list = await cache.get("All_Dish")
    if all_dish_list:
        ...
        await cache.set("All_Dish", all_dish_list)
    key_name = f"Order_User_{user['id']}_Dish_{dish_id}"
    order = await db.execute(
        _sql.select(table.Order).filter_by(user_id=user["id"], dish_id=dish_id)
    )
    ...
    if not order:
        if quantity <= 0:
            raise _fastapi.HTTPException(
                status_code=400,
                detail="the number of dishes ordered must be greater than zero",
            )
